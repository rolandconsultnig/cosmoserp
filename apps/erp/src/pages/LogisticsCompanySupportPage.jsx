...
export default function LogisticsCompanySupportPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await logisticsJson('/logistics/company/support/tickets');
      setTickets(res.data || []);
    } catch (e) {
  ...
  const submit = async (e) => {
    ...
    setSaving(true);
    try {
      await logisticsJson('/logistics/company/support/tickets', {
        method: 'POST',
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: 'rgba(255,255,255,0.40)' }}>
              Open a ticket for platform or integration issues. Your company team can track submissions here.
            </p>
          </div>
        </div>
        <Link to="/logistics/company" className="text-[12px] font-bold text-blue-400 hover:underline">Overview</Link>
      </div>
