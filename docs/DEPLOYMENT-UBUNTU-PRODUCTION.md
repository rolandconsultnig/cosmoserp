...
- **502 Bad Gateway on `/api`** (e.g. `/api/marketplace/listings`, `/api/marketplace/auth/login`):
  1. **API must be running on port 5133** — Nginx proxies `/api` to `http://127.0.0.1:5133`. If the API is down or on another port, you get 502.
  2. **Check PM2**: `pm2 list` — ensure `cosmos-api` (or your API process) is **online**. If not: `cd /root/cosmoserp/apps/api` (or your path), then `pm2 start src/index.js --name cosmos-api` (or `pm2 restart cosmos-api`).
  3. **Check PORT in .env**: In `apps/api/.env` set `PORT=5133`. Without this, the API may listen on 5000 and Nginx will get "connection refused" → 502.
  4. **Test API directly on the server**: `curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:5133/health` or `curl -s "http://127.0.0.1:5133/api/marketplace/listings?limit=1"`. If this fails, the API is not listening on 5133 or has crashed — check `pm2 logs cosmos-api` and `pm2 restart cosmos-api`.
- **500 on `/api/marketplace/auth/register` or `/api/marketplace/auth/login`**: Follow the step-by-step runbook: **[docs/PRODUCTION-RUNBOOK-MARKETPLACE-AUTH.md](PRODUCTION-RUNBOOK-MARKETPLACE-AUTH.md)**. In short: set `JWT_SECRET` (and `DATABASE_URL`, `PORT=5133`) in `apps/api/.env`, run `npx prisma migrate deploy`, then `pm2 restart cosmos-api --update-env`.
  5. **Nginx config**: Ensure `location /api { proxy_pass http://127.0.0.1:5133; ... }` and run `sudo nginx -t && sudo systemctl reload nginx`.
